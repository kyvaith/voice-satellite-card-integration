...
| **Type** | **Black overlay** - solid black overlay. **Media** - image, video, folder, or camera feed selected from the HA media library. **Website** - embed any URL in an iframe (e.g. immich-kiosk, photo frame apps, a dashboard). **WebRTC Camera** - low-latency camera stream from a go2rtc WebRTC/WHEP endpoint |
...
| **WebRTC camera URL** *(WebRTC Camera type only)* | go2rtc WebRTC/WHEP endpoint, e.g. `http://homeassistant.local:1984/api/webrtc?src=front_door`. The stream plays muted, full screen, with sub-second latency, and reconnects automatically if the connection drops. go2rtc must allow cross-origin requests: add `api:` / `origin: "*"` to `go2rtc.yaml` and restart go2rtc, otherwise the browser blocks the request with a CORS error. Also, if Home Assistant is served over HTTPS, the go2rtc endpoint must be HTTPS too or the browser will block it as mixed content |
